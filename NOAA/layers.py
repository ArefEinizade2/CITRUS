...
#%%
class TimeThenSpaceModel_v2(nn.Module):
    def __init__(self, input_size: int, n_nodes: int, horizon: int,
                 ...
                 last_activation=None, mlp_hidden_dims=None, dropout=True, with_MLP=True, 
                 diffusion_method='spectral', device='cpu', graph_wise=False):
        super(TimeThenSpaceModel_v2, self).__init__()
...
class CITRUS(nn.Module):
    def __init__(self, input_size: int, n_nodes: int, horizon: int,
        ...
        #                          root_weight=True)
        #CPGNN:
        self.CPGNN = CPGNN_ST_in_TTS(None, hidden_size, hidden_size, None, None, 
                     mass, evals, evecs, C_width, N_block, single_t, use_gdc, num_nodes, 
                     last_activation, mlp_hidden_dims, dropout, with_MLP, 
                     diffusion_method, device, graph_wise)
    ...
    def forward(self, x):
        # print(x.shape)
        # x: [batch time nodes features]
        # x = torch.reshape(x, [x.shape[0], 32, 10, 1]).transpose(2, 1)
        x = x.permute([0, 3, 2, 1])

        # x = x.view(x.shape[0], 10, 32, 1)
        # print(x.shape)
        b, t, n, f = x.size()
        # print('1: ', x.shape)
        # Concatenate node embeddings to input
        ...
        x_out = self.decoder(z_emb)  # linear proj: z=[b n f] -> x_out=[b n t⋅f]
        # print('2: ', x_out.shape)
        x_horizon = self.rearrange(x_out).squeeze()
        # print('3: ', x_horizon.shape)
        # print(x_horizon.shape)
        return x_horizon
#%%
...
class GTCNN(torch.nn.Module):
    def __init__(self, K, F_in, F_out, num_layers, Adj_t, Adj_s):
        ...
        self.Adj_t = Adj_t
        self.Adj_s = Adj_s
        self.s = nn.Parameter(torch.Tensor(4))
    ...
    def forward(self, x):
        ...
        for l in range(self.num_layers):
            ...
            for k in range(self.K):
                ...
                Sum = Sum + torch.matmul(torch.matmul(torch.matrix_power(self.Adj, k), x), H_kl)
            x = torch.tanh(Sum) # --> b t*n f
        x = self.lin_last(x) # --> b t*n f
        return x
#%%
import torch
...
class GRUGCNModel(BaseModel):
    ...
    def forward(self,
        ...
        for layer in self.gcn_layers:
            out = layer(out, self.edge_index)

        out = out + self.skip_con(x)
        # out = out.unsqueeze(-1)
        # print(out.shape)
        out = self.readout(out).squeeze()
        # print(out.shape)
        return out
